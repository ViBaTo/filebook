...
export default function CreatePage() {
  ...
  const processAndUpload = useCallback(
    async (bookId: string, file: File) => {
      try {
        ...
        const onRenderProgress = (progress: RenderProgress) => {
          setProcessingStatus(
            progress.status === 'complete' ? 'uploading' : 'rendering'
          )
          setCurrentPage(progress.currentPage)
          setTotalPages(progress.totalPages)
        }

        const renderedPages = await renderPDFToImages(pdfData, onRenderProgress)
  ...
  const handleUploadComplete = useCallback(
    (bookId: string, slug: string, pdfUrl: string) => {
      if (pdfFile) {
        processAndUpload(bookId, pdfFile)
      }
    },
    [pdfFile, processAndUpload]
  )
    ...
    <div className='min-h-screen bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f3460]'>
      ...
      <header className='p-6'>
        ...
        >
          ...
          <span className='font-semibold'>FlipBook</span>
        </a>
      </header>
        ...
        {step === 'upload' && (
          <div>
            ...
            <p className='text-center text-sm text-gray-500 mt-6'>
              Gratis: Hasta 350MB (temporal), 50 páginas.{' '}
